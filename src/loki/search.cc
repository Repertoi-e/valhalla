...
bool search_filter(const DirectedEdge* edge,
                   ...
                   const valhalla::baldr::Location::SearchFilter& filter) {
...
bool side_filter(const PathLocation::PathEdge& edge,
  ...
  return same != (location.preferred_side_ == valhalla::baldr::Location::PreferredSide::SAME);
}

bool heading_filter(const valhalla::baldr::Location& location, float angle) {
...
bool layer_filter(const valhalla::baldr::Location& location, int8_t layer) {
...
struct projector_wrapper {
  projector_wrapper(const valhalla::baldr::Location& location, GraphReader& reader)
  ...
  valhalla::baldr::Location location;
...
struct bin_handler_t {
  ...
  bin_handler_t(const std::vector<valhalla::baldr::Location>& locations,
    ...
    std::unordered_set<valhalla::baldr::Location> uniq_locations(locations.begin(), locations.end());
  ...
  void correlate_node(const valhalla::baldr::Location& location,
  ...
  void correlate_edge(const valhalla::baldr::Location& location,
                      const candidate_t& candidate,
                      PathLocation& correlated,
                      std::vector<PathLocation::PathEdge>& filtered) {
  ...
  std::unordered_map<valhalla::baldr::Location, PathLocation> finalize() {
    ...
    std::unordered_map<valhalla::baldr::Location, PathLocation> searched;
    for (auto& pp : pps) {
      ...
      if ((pp.location.stoptype_ == valhalla::baldr::Location::StopType::THROUGH ||
           pp.location.stoptype_ == valhalla::baldr::Location::StopType::BREAK_THROUGH) &&
