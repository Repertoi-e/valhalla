...
std::vector<std::string> openlr_edges(const TripLeg &leg) {
  // TODO: can we get the uncompressed shape when we have it in other
  // serialization steps
  ...
  for (const TripLeg::Node &node : leg.node()) {
    ...
    float forward_heading = midgard::tangent_angle(
        begin_index, start, shape, 20.f, true, begin_index, end_index);
    const auto &end = shape[end_index];
    float reverse_heading = midgard::tangent_angle(
        end_index, end, shape, 20.f, false, begin_index, end_index);

    std::vector<baldr::OpenLR::LocationReferencePoint> lrps;
    lrps.emplace_back(start.lng(), start.lat(), forward_heading, frc, fow,
                      nullptr,
                      edge.length_km() * valhalla::midgard::kMetersPerKm, frc);
    lrps.emplace_back(end.lng(), end.lat(), reverse_heading, frc, fow,
                      &lrps.back());
...
std::string serializeStatus(Api &request) {
  ...
  status_doc.AddMember(
      "version",
      rapidjson::Value().SetString(request.status().version(), alloc), alloc);
  status_doc.AddMember(
      "tileset_last_modified",
      rapidjson::Value().SetInt(request.status().tileset_last_modified()),
      alloc);

  rapidjson::Value actions_list(rapidjson::kArrayType);
  for (const auto &action : request.status().available_actions()) {
    actions_list.GetArray().PushBack(
        rapidjson::Value{}.SetString(action.c_str(), alloc), alloc);
  }
  status_doc.AddMember("available_actions", actions_list, alloc);

  if (request.status().has_has_tiles_case())
    status_doc.AddMember(
        "has_tiles", rapidjson::Value().SetBool(request.status().has_tiles()),
        alloc);
  if (request.status().has_has_admins_case())
    status_doc.AddMember(
        "has_admins", rapidjson::Value().SetBool(request.status().has_admins()),
        alloc);
  ...
  if (request.status().has_has_transit_tiles_case())
    status_doc.AddMember(
        "has_transit_tiles",
        rapidjson::Value().SetBool(request.status().has_transit_tiles()),
        alloc);
  // a 0 changeset indicates there's none, so don't write in the output
  // TODO: currently this can't be tested as gurka isn't adding changeset IDs to
  // OSM objects (yet)
  if (request.status().has_osm_changeset_case() &&
      request.status().osm_changeset())
    status_doc.AddMember(
        "osm_changeset",
...
void route_references(json::MapPtr &route_json, const TripRoute &route,
                      const Options &options) {
  const bool linear_reference = options.linear_references() &&
                                (options.action() == Options::trace_route ||
                                 options.action() == Options::route);
  if (!linear_reference) {
    return;
  }
...
void openlr(const valhalla::Api &api, int route_index,
            rapidjson::writer_wrapper_t &writer) {
  // you have to have requested it and you have to be some kind of route
  // response
  if (!api.options().linear_references() ||
      (api.options().action() != Options::trace_route &&
       api.options().action() != Options::route))
    return;
...
void serializeWarnings(const valhalla::Api &api,
                       rapidjson::writer_wrapper_t &writer) {
...
json::ArrayPtr serializeWarnings(const valhalla::Api &api) {
  ...
  for (const auto &warning : api.info().warnings()) {
    warnings->emplace_back(
        json::map({{"code", warning.code()}, {"text", warning.description()}}));
  }
  return warnings;
}
...
std::string serializePbf(Api &request) {
  ...
  // if they dont want the options object but its a service request we have to
  // work around it
  bool skip_options = !request.options().pbf_field_selector().options() &&
                      request.has_info() && request.info().is_service();
  Options dummy;
  if (skip_options) {
    request.mutable_options()->Swap(&dummy);
  }

  // disable all the stuff we need to disable, options must be last since we are
  // referencing it
  if (!selection.trip())
    request.clear_trip();
...
void geojson_shape(const std::vector<midgard::PointLL> &shape,
                   rapidjson::writer_wrapper_t &writer) {
...
// Serialize a location (waypoint) in OSRM compatible format. Waypoint format is
// described here:
...
valhalla::baldr::json::MapPtr waypoint(const valhalla::Location &location,
                                       bool is_tracepoint, bool is_optimized) {
  ...
  auto loc = json::array({});
  loc->emplace_back(
      json::fixed_t{location.correlation().edges(0).ll().lng(), 6});
  loc->emplace_back(
      json::fixed_t{location.correlation().edges(0).ll().lat(), 6});
  waypoint->emplace("location", loc);

  // Add street name.
  std::string name = location.correlation().edges_size() &&
                             location.correlation().edges(0).names_size()
  ...
  if (is_tracepoint) {
    ...
    if (location.correlation().waypoint_index() ==
        numeric_limits<uint32_t>::max()) {
      // when tracepoint is neither a break nor leg's starting/ending
      // point (shape_index is uint32_t max), we assign null to its
      // waypoint_index
    ...
    waypoint->emplace(
        "matchings_index",
        static_cast<uint64_t>(location.correlation().route_index()));
  }

  // If the location was used for optimized route we add trips_index and
  // waypoint index (index of the waypoint in the trip)
  if (is_optimized) {
...
void waypoint(const valhalla::Location &location,
              rapidjson::writer_wrapper_t &writer, bool is_tracepoint,
              bool is_optimized) {
  // Output location as a lon,lat array. Note this is the projected
  // lon,lat on the nearest road.
  writer.start_object();
...
json::ArrayPtr intermediate_waypoints(const valhalla::TripLeg &leg) {
  ...
  for (const auto &loc : leg.location()) {
    // Only create via_waypoints object if the locations are via or through
    // types
    if (loc.type() == valhalla::Location::kVia ||
        loc.type() == valhalla::Location::kThrough) {
      auto via_waypoint = json::map({});
      via_waypoint->emplace(
          "geometry_index",
...
void serializeIncidentProperties(
    rapidjson::writer_wrapper_t &writer,
    const valhalla::IncidentsTile::Metadata &incident_metadata,
    const int begin_shape_index, const int end_shape_index,
    const std::string &road_class, const std::string &key_prefix) {
  writer(key_prefix + "id", std::to_string(incident_metadata.id()));
  // Type is mandatory
  writer(key_prefix + "type",
         valhalla::incidentTypeToString(incident_metadata.type()));
  if (!incident_metadata.iso_3166_1_alpha2().empty()) {
    writer(key_prefix + "iso_3166_1_alpha2",
           incident_metadata.iso_3166_1_alpha2());
  }
  if (!incident_metadata.iso_3166_1_alpha3().empty()) {
    writer(key_prefix + "iso_3166_1_alpha3",
           incident_metadata.iso_3166_1_alpha3());
  }
  ...
  if (!incident_metadata.long_description().empty()) {
    writer(key_prefix + "long_description",
           incident_metadata.long_description());
  }
  if (incident_metadata.creation_time()) {
    writer(key_prefix + "creation_time",
           baldr::DateTime::seconds_to_date_utc(
               incident_metadata.creation_time()));
  }
  if (incident_metadata.start_time() > 0) {
    writer(key_prefix + "start_time", baldr::DateTime::seconds_to_date_utc(
                                          incident_metadata.start_time()));
  }
  ...
  if (!incident_metadata.sub_type_description().empty()) {
    writer(key_prefix + "sub_type_description",
           incident_metadata.sub_type_description());
  }
  ...
  if (incident_metadata.num_lanes_blocked()) {
    writer(key_prefix + "num_lanes_blocked",
           incident_metadata.num_lanes_blocked());
  }
